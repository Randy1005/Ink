...
TEST_CASE("Insert / Remove / Update  Edges (one inc iteration)" * doctest::timeout(300)) {
  ...
  //
  //ink_inc.insert_edge("v4", "v6", 6);  
  //inc_full.insert_edge("v4", "v6", 6);  
  ...
  //ink_inc.insert_edge("v6", "v10", 2);
  //inc_full.insert_edge("v6", "v10", 2);

  ink_inc.report_incremental_v2(4, true, false);   
...
TEST_CASE("Insert / Remove / Update Edges (one inc iteration)" * doctest::timeout(300)) {
  ...
  inc_full.insert_edge("v8", "v12", 11);
	...
	// report once with incsfxt, save the pfxt nodes
	ink_inc.report_incsfxt(20, true, false);
  auto costs_inc = ink_inc.get_path_costs();
  inc_full.report_incsfxt(20, false, false);
  auto costs_full = inc_full.get_path_costs();
  ...
  ink_inc.report_incremental_v2(20, false, false);   
  costs_inc = ink_inc.get_path_costs();
...
//  }
//
//  // TODO: go over the logic of mark_pfxt_nodes again
//  // since we're reusing the slots in eptrs
//  // we may have inconsistent sfxt.links and pfxt_node.edge
//  // now I've nullified pfxtnode.edge to indicate a removed edge
//  // but not sure what issues might rise in the future
//
//  // chain 1: insert buffers
